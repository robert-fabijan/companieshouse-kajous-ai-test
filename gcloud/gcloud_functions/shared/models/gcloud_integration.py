...
class GCloudIntegration:
    ...
    def _get_google_cloud_client(self, credentials) -> storage.client.Client:
        '''
        ...
        try:
            ...
            return storage.client.Client.from_service_account_json(credentials_dict) # return client if there is a api key provided
    ...
    def upload_data_to_cloud_from_file(self, secret, bucket_name, response, blob_name, filename):
        ''' Uploads files with api data to GCP buckets. '''
        bucket = self._get_google_cloud_client(secret).bucket(bucket_name) # connect to bucket
        ...
        # Use BytesIO to stream the content to the blob
